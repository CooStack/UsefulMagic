...
class MagicSwordSkill : Skill, SkillCondition, SkillCancelCondition {
    ...
    override fun onRelease(source: LivingEntity, holdingTick: Int) {
        ...
        ServerCameraUtil.sendShake(source.world as ServerWorld, source.pos, 64.0, 0.5, 10)
        val shoot = DirectionShootEmitters(source.pos, source.world).apply {
        ...
        CooParticleAPI.scheduler.runTaskTimerMaxTick(5, barrages.size * 5) {
            if (!iterator.hasNext()) {
            ...
            // 设置朝向
            it.target = target
            it.direction = it.loc.relativize(target.pos)
            it.world.playSound(
                null,
                it.loc.x,
                it.loc.y,
    ...
    override fun holdingTick(holdingEntity: LivingEntity, holdTicks: Int) {
        ...
        while (iterator.hasNext()) {
            ...
            val barrage = it.key
            barrage.loc = holdingEntity.pos.add(it.value.toVector())
            if (!barrage.valid) {
                iterator.remove()
            }
        }
    ...
    override fun canTrigger(entity: LivingEntity): Boolean {
        return entity is MobEntity && entity.target != null
    }

...
class EndRodExplosionStyle(uuid: UUID = UUID.randomUUID()) : ParticleGroupStyle(512.0, uuid) {
    ...
    override fun getCurrentFrames(): Map<StyleData, RelativeLocation> {
        ...
        while (currentStep < ballRadius) {
            ...
            val currentCount =
                MathHelper.lerp(progress, minBallCountPow.toDouble(), maxBallCountPow.toDouble()).roundToInt()
            ...
            res.putAll(
                builder.createWithStyleData {
                    StyleData {
                        ...
                        if (randomMaxAge) {
                            this.maxAge = max(20, this@EndRodExplosionStyle.maxAge - random.nextInt(60)) - 10
                        } else {
                            this.maxAge = this@EndRodExplosionStyle.maxAge
                        }

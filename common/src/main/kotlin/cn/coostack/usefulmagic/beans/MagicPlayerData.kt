...
class MagicPlayerData(var owner: UUID) {
    ...
    fun canCost(mana: Int, client: Boolean): Boolean {
        val playerInstance = if (client) {
            ...
            UsefulMagic.server.playerList.getPlayer(owner)
        }
        return this.mana >= mana || playerInstance?.isCreative ?: false
    }
    ...
    fun tick() {
        // 确保玩家在线
        UsefulMagic.server.playerList.getPlayer(owner) ?: return
        if (tick++ % 20 != 0) {
            return
        }
        tick = 1

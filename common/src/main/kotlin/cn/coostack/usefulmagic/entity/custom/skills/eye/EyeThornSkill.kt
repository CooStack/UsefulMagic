...
import cn.coostack.usefulmagic.entity.util.phases.eye.EyeOrbitCloserPhase
    ...
    private fun prickingOnPosition(position: Vec3, world: Level, shooter: LivingEntity, fast: Boolean = false) {
        ...
        CooParticlesAPI.scheduler.runTask(if (fast) 8 else 16) {
            ...
            ServerSoundManager.instance(
                UsefulMagicSoundEvents.THORN_HIT.get(), SoundSource.HOSTILE
            ).layer("thorn_hit")
                .uniqueKey(UUID.randomUUID().toString())
                .pitch(0.9f + Random.nextFloat() * 0.3f)
                .world(world as ServerLevel)
                .position(position)
                .volume(0.5f)
                .visibleRange(24.0)
                .spawn()

...
class TrackedPointBarrage(
    ...
    var refiner: Double = 3.7
    var particleMaxAge: Int = 4
    var particleMinAge: Int = 8
    var leftColor = Math3DUtil.colorOf(255, 212, 88)
    var rightColor = Math3DUtil.colorOf(117, 109, 156)
    var startTrackingTick = 10
    var trackingTick = 10
    var tick = 0
    ...
    override fun filterHitEntity(livingEntity: LivingEntity): Boolean {
        ...
        if (!EntityUtil.filterDragon.test(livingEntity)) {
            return false
        }
        if (!EntityUtil.filterEye.test(livingEntity)) {
            return false
        }
    ...
    override fun tick() {
        if (tick++ >= startTrackingTick && !startTracking) {
            startTracking = true
            tick = 0
            return
        }
        if (tick < trackingTick && startTracking) {
            // 玩家吸引力

package cn.coostack.usefulmagic.particles.style.wand.anti
...
class AntiEntityWandStyle(
    ...
    override fun onDisplay() {
        ...
        addPreTickAction {
            ...
            // 处理旋转
            // 修改target -> 追踪实体
            val entities = world!!.getEntitiesOfClass<LivingEntity>(
                LivingEntity::class.java, AABB.ofSize(pos, 256.0, 256.0, 256.0)
            ) {
                it.uuid != bindPlayer && it.isAlive && !it.noPhysics
            }
            val nearest = entities.minByOrNull { it.position().distanceTo(pos) } ?: world?.getPlayerByUUID(bindPlayer)
            ?: return@addPreTickAction
            // 取 target
            val dir = pos.relativize(nearest.position())

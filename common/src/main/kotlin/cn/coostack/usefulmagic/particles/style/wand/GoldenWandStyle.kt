package cn.coostack.usefulmagic.particles.style.wand
...
class GoldenWandStyle(val usingPlayerUUID: UUID, uuid: UUID = UUID.randomUUID()) :
            ...
            ParticleDisplayer.withStyle(
                ParticleShapeStyle(uuid = it)
                    ...
                    .toggleBeforeDisplay { map ->
                        ...
                        this.preRotateTo(
                            map,
                            RelativeLocation.of(usingPlayer?.forward ?: Vec3(0.0, 1.0, 0.0))
                        )
                    }.toggleOnDisplay {
    ...
    override fun beforeDisplay(styles: Map<StyleData, RelativeLocation>) {
        ...
        preRotateTo(
            styles,
            RelativeLocation.of(usingPlayer.forward)
        )
    }
    ...
    override fun onDisplay() {
        addPreTickAction {
            val usingPlayer = world!!.getPlayerByUUID(usingPlayerUUID) ?: return@addPreTickAction
            rotateParticlesToPoint(RelativeLocation.of(usingPlayer.forward))
            teleportTo(usingPlayer.eyePosition)
            if (!client) {
                if (statusHelper.displayStatus != 2 && !usingPlayer.useItem.isOf(UsefulMagicItems.GOLDEN_WAND)) {

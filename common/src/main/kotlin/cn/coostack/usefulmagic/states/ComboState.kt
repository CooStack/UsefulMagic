...
class ComboState(val owner: UUID) {
     ...
     */
    fun increaseIf(predicate: (owner: Player) -> Boolean) {
        val player = UsefulMagic.server.playerList.getPlayer(owner) ?: return
        if (predicate(player)) {
            increase()
        }
    }
     ...
     */
    fun increaseIf(comboTime: Int, predicate: (owner: Player) -> Boolean) {
        val player = UsefulMagic.server.playerList.getPlayer(owner) ?: return
        if (predicate(player)) {
            increase(comboTime)
        }
